...
export function useEmploymentWizard() {
  ...
  const completeWizard = useCallback(() => setState((current) => { const completedAt = new Date().toISOString(); const next = { ...current, status: 'completed' as const, completedAt }; persist(next, true); void wizardService.complete('employment', current.data); return next }), [persist])
